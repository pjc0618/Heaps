...
class HeapTest {
     ...
     *  This is the case if:
     *    (1) c.length, mh.size, and mh.dict.size() are all equal.
     *    (2) for each i in 0..size-1, (c[i], p[i]) is the entry mh.d[i]
     *    (3) For each i in 0..size-1, (c[i], i) is in dict.
     *    
    ...
    public <T> void check(T[] c, double p[], Heap<T> mh) {
        ...
        assertEquals(c.length, mh.dict.size());
        ...
        for (int k= 0; k < c.length; k= k+1) {mhS.add(mh.dict.get(c[k]));}
        assertEquals(s, mhS);
    }
    ...
    public void check(Heap<Integer> m1, Heap<Integer> m2) {
        ...
        assertEquals(m1.size, m2.dict.size());
        ...
        //check data fields
        assertTrue(m1.dict.equals(m2.dict));
    }

    /** = a list of values in c, separated by ", " and delimited by "[", "]" */
    ...
    public Heap<String> minHeapify(String[] c, double[] p) {
        Heap<String> m= new Heap<String>(false);
        for (int h= 0; h < c.length; h= h+1) {
            m.insert(c[h], p[h]);
        }
        return m;
    ...
    /** Return a max-heap with values c without using insert.
     * Values used as priorities */
    ...
    /** Return a min-heap with values c and priorities p without using insert. */
    public Heap<Integer> griesHeapify(Integer[] c, double[] p) {
        Heap<Integer> heap= new Heap<Integer>(false);
        heap.c= heap.createVPArray(c.length); //new Entry[c.length];
        for (int k= 0; k < c.length; k= k+1 ) {
            heap.c[k]= heap.new data(c[k], p[k]);
            heap.dict.put(c[k], k);
        }
        heap.size= c.length;
        return heap;
    ...
    /** Return a max-heap with values c and priorities p without using insert. */
    ...
    @Test
    /** Test whether insert works in a min-heap when the priority of the
     *  value being inserted is >= priorities of other values in the heap.
    ...
    public void test01insertException() {
        ...
        assertThrows(IllegalArgumentException.class, () -> {mh2.insert(5, 5.0);});
    }
    ...
    public void test10ensureSpace() {
        Heap<Integer> mh= new Heap<Integer>(false);
     ...
     @Test
    /** Test that inserting integers 0..9 into a heap, with priorities
    ...
    public void test11ensureSpace() {
     ...
     @Test
    /** Test that inserting integers 0..10 into a heap, with priorities
    ...
    public void test70Strings() {
        Heap<String> mh= new Heap<String>(false);
    ...
    public void test90bigTests() {
        ...
        // Build the Heap and dict to be able to get priorities easily
    ...
    /** Poll all elements of m into c.
     * Precondition m and c are the same size. */
